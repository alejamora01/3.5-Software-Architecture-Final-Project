...
`FTPClient` calls `createFileEntryParser(key)` to get a parser for the connected server's OS. The factory inspects the SYST response string and instantiates the correct concrete parser (Unix, NT, VMS, OS/400, etc.) without the client knowing which one it received.
